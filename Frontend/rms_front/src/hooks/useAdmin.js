...
export function useAdmin() {
    ...
    async function fetchAdmins(user) {
        ...
        const response = await axios.post(`${API_URL}/admins/login`, user, {
    ...
    async function createAdmin(user) {
        const response = await axios.post(`${API_URL}/admins/`, user);
        setAdmins(prev => [...prev, response.data]);
        return response;
    }
    
    async function forgotPassword(email) {
        const response = await axios.post(`${API_URL}/admins/forgot-password`, email);
        setAdmins(response.data);
        return response.data;
    }

    async function resetPassword(password, token) {
        const response = await axios.post(`${API_URL}/admins/reset-password`, { password, token });
        setAdmins(response.data);
        return response.data;
    }
